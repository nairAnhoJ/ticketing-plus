...
function AvgResolutionByUser({ tickets } : Props) {
	const users = [
		...new Set(
			tickets
  ...
  const resolutionByUser = users.map((user, i) => {
    const resolved = tickets.filter(t => t.completed_by === user && t.completed_at);
  ...
  return (
    <ChartCard title="Avg. Resolution Time by User" subtitle="Average hours to resolve a ticket per user">
...
export default AvgResolutionByUser